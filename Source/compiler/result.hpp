...
private:
    ...
    vector<string> property_values;
    
public:
    result();
    result(map< string, vector<double> > v1, map< string, vector< map<string, string> > > v2, vector<string> v3);
    
    map< string, vector<double> > get_digit_values();
    map< string, vector< map<string, string> > > get_draw_text_values();
    vector<string> get_property_values();
};
